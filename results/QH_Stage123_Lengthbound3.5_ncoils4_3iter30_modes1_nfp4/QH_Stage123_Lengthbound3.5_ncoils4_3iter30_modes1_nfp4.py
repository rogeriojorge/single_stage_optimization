...
LENGTHBOUND_QA = 5.4 # Threshold for the length of each coil
...
maxj_target = -5.0 #Target value for the derivatives
maxj_weight = 5e0 #Weight of the derivatives in the stage 1 objective function
snorms_maxj = [5/16] # Flux surfaces at which the derivative will be calculated
nlambda = 6 #Number of different values of lambda to test
nphi_maxj = 22 #Resolution in the phi angle to build the 2D grid in theta and phi angle
ntheta_maxj = 32 #Resolution in the theta angle to build the 2D grid in theta and phi angle 
...
phi_max_maxj = 10#20 #Maximum value of phi angle for obtaining turning points (v|| = 0)
pppi = 22 #Points per pi rad
eps = 1e-3 #Difference between points for the calculation of the numerical derivative
...
# Frequent use
max_modes = [1, 2, 2] # Fourier mode resolution for the plasma surface
...
MAXITER_stage_2 = 100 # Number of iterations to perform in the stage 2 optimization
MAXITER_single_stage = 30 # Number of iterations to perform in the main optimization loop
# Flags
...
output_interval = 10 # Number of single stage steps in between coil and surface outputs to file
...
qsqi_weight = 1e-1 # Weight for the quasisymmetry residuals objective
